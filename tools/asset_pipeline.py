...
def validate_outputs():
    ...
    if not SPRITES_JSON.exists():
        print("Error: assets/sprites.json missing after slicing.")
        return False, 0, 0
    ...
    for name, sprite_data in sprites.items():
        for frame in sprite_data.get("frames", []):
            path = REPO_ROOT / frame.get("path")
            if not path.exists():
                print(f"Warning: Frame missing on disk: {frame.get('path')}")
            else:
                frame_count += 1
...
def main():
    ...
    print(f"{sheets} sprite sheets → {frames} individual frames")
